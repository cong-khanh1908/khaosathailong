...
function collectForm(type) {
  ...
  // FIX BUG-06: Gắn khoaId từ config vào record để byt.js dùng khi gửi
  ...
  def.sections.forEach(sec=>{
    sec.questions.forEach((q,qi)=>{
      ...
      // FIX BUG-04: Lưu value=0 (KSD) thay vì null để phân biệt "không trả lời" vs "KSD"
...
function saveForm(type) {
  ...
  if (navigator.onLine && gsReady()) {
    setTimeout(async () => {
      ...
      // THÊM: Ghi vào tab BYT_QUEUE cho Hailong Upload Engine (chỉ m1/m2)
      if (rec.type === 'm1' || rec.type === 'm2') {
        const qOk = await gsPushSurveyToBytQueue(rec);
        if (qOk) {
          toast('📋 Đã thêm vào BYT_QUEUE – dùng Hailong Upload Engine để gửi lên BYT', 'info');
  ...
  // FIX: Chỉ tự gửi nếu đã đăng nhập BYT (tránh race condition và loop vô hạn)
