...
function exportCSV(){
  ...
  // FIX BUG-10: answered bao gồm 0, avg chỉ tính >0
const rows=DB.surveys.map(r=>{const answered=r.answers?.filter(a=>a.value!==null&&a.value!==undefined&&a.value!=='')||[];const scored=answered.filter(a=>Number(a.value)>0);const avg=scored.length?(scored.reduce((s,a)=>s+Number(a.value),0)/scored.length).toFixed(2):'';return[r.id,r.createdAt,SURVEYS[r.type]?.label||r.type,SURVEYS[r.type]?.qd||'',r.ngay||'',r.khoa||r.donvi||'',r.gt||'',r.tuoi||'',avg,answered.length,r.answers?.length||0,r.submittedBy||'',r.status];});
...
function showPage(name){
  ...
  if(name==='bytupload'){renderBYTQueue();loadAutoUploadCheckboxes();if(typeof showBYTConfigStatus==='function')showBYTConfigStatus();}
  if(name==='hailong'){if(typeof renderHailongPage==='function')renderHailongPage();}
