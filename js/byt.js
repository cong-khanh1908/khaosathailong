...
// =========================================================
// STATUS DASHBOARD – BYT_QUEUE (THÊM v14)
// Thay thế UI "Gửi phiếu trực tiếp từ kshl-main" bằng màn hình đọc trạng thái
// — việc gửi thực tế do Hailong Upload Engine (ứng dụng Electron riêng) đảm nhiệm.
// =========================================================
function openBYTQueueSheet() {
  const sid = typeof CFG !== 'undefined' ? CFG.sheetid : '';
  if (!sid) { toast('⚠️ Chưa cấu hình Spreadsheet ID', 'warning'); return; }
  window.open(`https://docs.google.com/spreadsheets/d/${sid}/edit`, '_blank');
}

async function refreshBYTStatusDash() {
  const el = document.getElementById('byt-status-dash');
  if (!el) return;
  if (typeof gsReady === 'undefined' || !gsReady()) {
    el.innerHTML = '<div class="warn-box" style="font-size:12.5px;">⚠️ Chưa cấu hình Google Sheet. Vào <b>Cấu hình hệ thống</b> → Điền Spreadsheet ID để kết nối.</div>';
    return;
  }
  el.innerHTML = '<div style="text-align:center;padding:16px;color:var(--text3);">⏳ Đang tải từ Google Sheet...</div>';
  try {
    const data = await gsFetchBytQueueStatus();
    renderBYTStatusDash(data);
  } catch(e) {
    el.innerHTML = `<div class="warn-box" style="color:var(--accent2);">❌ Lỗi: ${e.message}</div>`;
  }
}

function renderBYTStatusDash(data) {
  const el = document.getElementById('byt-status-dash');
  if (!el) return;

  if (!data) { refreshBYTStatusDash(); return; }

  if (data.tabMissing) {
    el.innerHTML = `<div class="warn-box" style="font-size:12.5px;">
      ⚠️ Tab <code>BYT_QUEUE</code> chưa tồn tại trên Google Sheet.
      Sẽ được tạo tự động khi nhân viên lưu phiếu Mẫu 1/2 lần đầu tiên, hoặc khi Hailong Upload Engine bấm "Kiểm tra kết nối".
    </div>`;
    return;
  }

  const totalPending = (data.pendingNoiTru||0) + (data.pendingNgoaiTru||0);
  const cards = [
    { label:'Nội trú Pending', value: data.pendingNoiTru||0, color:'#E65100', bg:'#FFF3E0' },
    { label:'Ngoại trú Pending', value: data.pendingNgoaiTru||0, color:'#F57C00', bg:'#FFF8E1' },
    { label:'Đã gửi thành công', value: data.successCount||0, color:'#2E7D32', bg:'#E8F5E9' },
    { label:'Gửi thất bại', value: data.failedCount||0, color:'#C62828', bg:'#FFEBEE' },
  ];
  const cardsHtml = `<div style="display:grid;grid-template-columns:repeat(4,1fr);gap:10px;margin-bottom:14px;">
    ${cards.map(c=>`<div style="background:${c.bg};border-radius:8px;padding:12px;text-align:center;">
      <div style="font-size:24px;font-weight:800;color:${c.color};">${c.value}</div>
      <div style="font-size:11px;color:#555;margin-top:2px;">${c.label}</div>
    </div>`).join('')}
  </div>`;

  let instructionHtml = '';
  if (totalPending > 0) {
    instructionHtml = `<div style="background:#E3F2FD;border-left:3px solid #1565C0;padding:10px 14px;border-radius:6px;font-size:12.5px;margin-bottom:10px;">
      📌 Có <b>${totalPending} phiếu Pending</b> (${data.pendingNoiTru||0} Nội trú + ${data.pendingNgoaiTru||0} Ngoại trú) đang chờ trong BYT_QUEUE.
      Mở <b>Hailong Upload Engine</b> → Tab "☁️ Upload từ Google Sheet" → Chọn loại phiếu → Bấm <b>Bắt đầu</b>.
    </div>`;
  } else if ((data.successCount||0) > 0) {
    instructionHtml = `<div style="background:#E8F5E9;border-left:3px solid #43A047;padding:10px 14px;border-radius:6px;font-size:12.5px;margin-bottom:10px;">
      ✅ Không có phiếu Pending nào. Tổng đã gửi thành công: <b>${data.successCount}</b> phiếu.
    </div>`;
  }

  el.innerHTML = cardsHtml + instructionHtml;

  if (data.recent && data.recent.length > 0) {
    const recentCard = document.getElementById('byt-queue-recent-card');
    if (recentCard) recentCard.style.display = '';
    const listEl = document.getElementById('byt-queue-recent-list');
    if (listEl) {
      const statusColor = {'Pending':'#E65100','Success':'#2E7D32','Failed':'#C62828'};
      const statusBg = {'Pending':'#FFF3E0','Success':'#E8F5E9','Failed':'#FFEBEE'};
      listEl.innerHTML = `<div style="overflow-x:auto;"><table style="width:100%;font-size:11.5px;border-collapse:collapse;">
        <thead><tr style="background:#f5f5f5;">
          <th style="padding:6px 8px;text-align:left;white-space:nowrap;">ID</th>
          <th style="padding:6px 8px;text-align:left;white-space:nowrap;">Loại</th>
          <th style="padding:6px 8px;text-align:left;white-space:nowrap;">Ngày tạo</th>
          <th style="padding:6px 8px;text-align:left;white-space:nowrap;">Trạng thái</th>
          <th style="padding:6px 8px;text-align:left;white-space:nowrap;">Thời gian gửi</th>
          <th style="padding:6px 8px;text-align:left;">Lỗi</th>
        </tr></thead>
        <tbody>
        ${data.recent.map(r=>`<tr style="border-bottom:1px solid #f0f0f0;">
          <td style="padding:5px 8px;font-family:monospace;color:#888;">${(r.id||'').slice(-8)}</td>
          <td style="padding:5px 8px;">${r.type==='noi_tru'?'🏥 Nội trú':'🚶 Ngoại trú'}</td>
          <td style="padding:5px 8px;white-space:nowrap;">${r.createdAt?new Date(r.createdAt).toLocaleString('vi-VN').slice(0,16):''}</td>
          <td style="padding:5px 8px;">
            <span style="background:${statusBg[r.status]||'#f5f5f5'};color:${statusColor[r.status]||'#333'};padding:1px 7px;border-radius:10px;font-size:11px;font-weight:600;">${r.status}</span>
          </td>
          <td style="padding:5px 8px;white-space:nowrap;color:#555;">${r.uploadTime||''}</td>
          <td style="padding:5px 8px;color:#C62828;font-size:11px;">${r.error||''}</td>
        </tr>`).join('')}
        </tbody>
      </table></div>`;
    }
...
// Hook vào showPage('bytupload') — showBYTConfigStatus được gọi từ ui.js
function showBYTConfigStatus() {
  refreshBYTStatusDash();
}

// Cập nhật badge "pendingBYTBadge" trên sidebar dựa trên BYT_QUEUE thực tế
// (thay cho cơ chế cũ đếm rec.bytStatus trên DB local — giờ DB local không
// còn lưu bytStatus nữa).
async function updateBYTPendingBadge() {
  const b = document.getElementById('pendingBYTBadge');
  if (!b) return;
  try {
    if (typeof gsReady === 'undefined' || !gsReady()) { b.style.display='none'; return; }
    const data = await gsFetchBytQueueStatus();
    const n = (data.pendingNoiTru||0) + (data.pendingNgoaiTru||0);
    b.textContent = n;
